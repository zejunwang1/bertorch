...
from bertorch.semantic_model import SimCSE
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=0.1, 
        type=float, 
        help="Dropout for pretrained model encoder."
    )
    parser.add_argument(
        "--pooling_mode", 
...
def train(args):
    ...
    texts = read_semantic_samples(args.train_data_file)
    train_dataset = SimcseDataset(texts, tokenizer, max_seq_length=args.max_seq_length)
    if args.local_rank == -1:
        train_dataloader = DataLoader(
            dataset=train_dataset,
            batch_size=args.batch_size,
            collate_fn=SimcseCollate(pad_token_id=tokenizer.pad_token_id),
            shuffle=True,
            pin_memory=True
        )
    else:
